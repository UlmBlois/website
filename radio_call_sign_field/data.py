COUNTRIES_PREFIX = (
    ("AF", "YA-"),  # Afghanistan
    ("AL", "ZA-"),  # Albania
    ("DZ", "7T-"),  # Algeria
    ("AD", "C3-"),  # Andorra
    ("AO", "D2-"),  # Angola
    ("AI", "VP-A"),  # Anguilla
    ("AG", "V2-"),  # Antigua and Barbuda
    ("AR", "LQ-"),  # Argentina
    ("AR", "LV-"),  # Argentina
    ("AM", "EK-"),  # Armenia
    ("AW", "P4-"),  # Aruba
    ("AU", "VH-"),  # Australia
    ("AT", "OE-"),  # Austria
    ("AZ", "4K-"),  # Azerbaijan
    ("BS", "C6-"),  # Bahamas
    ("BH", "A9C-"),  # Bahrain
    ("BD", "S2-"),  # Bangladesh
    ("BB", "8P-"),  # Barbados
    ("BY", "EW-"),  # Belarus
    ("BE", "OO-"),  # Belgium
    ("BZ", "V3-"),  # Belize
    ("BJ", "TY-"),  # Benin
    ("BM", "VP-B"),  # Bermuda
    ("BT", "A5-"),  # Bhutan
    ("BO", "CP-"),  # Bolivia
    ("BA", "T9-"),  # Bosnia and Herzegovina
    ("BW", "A2-"),  # Botswana
    ("BR", "PP-"),  # Brazil
    ("BR", "PR-"),  # Brazil
    ("BR", "PT-"),  # Brazil
    ("BR", "PU-"),  # Brazil
    ("VG", "VP-L"),  # British Virgin Islands
    ("BN", "V8-"),  # Brunei Darussalam
    ("BG", "LZ-"),  # Bulgaria
    ("BF", "XT-"),  # Burkina Faso
    ("BI", "9U-"),  # Burundi
    ("KH", "XU-"),  # Cambodia
    ("CM", "TJ-"),  # Cameroon
    ("CA", "CF-"),  # Canada
    ("CA", "C-F"),  # Canada
    ("CA", "C-G"),  # Canada
    ("CA", "C-I"),  # Canada
    ("CV", "D4-"),  # Cape Verde
    ("KY", "VP-C"),  # Cayman Islands
    ("CF", "TL-"),  # Central African Republic
    ("TD", "TT-"),  # Chad
    ("CL", "CC-"),  # Chile
    ("CN", "B-"),  # China
    ("TW", "B-"),  # Taiwan
    ("HK", "B-H"),  # Hong Kong
    ("HK", "B-K"),  # Hong Kong
    ("HK", "B-L"),  # Hong Kong
    ("MO", "B-M"),  # Macao
    ("CO", "HJ-"),  # Colombia
    ("CO", "HK-"),  # Colombia
    ("KM", "D6-"),  # Comoros
    ("CG", "TN-"),  # Congo
    ("CK", "E5-"),  # Cook Islands
    ("CD", "9Q-"),  # Democratic Republic of Congo
    ("CR", "TI-"),  # Costa Rica
    ("HR", "9A-"),  # Croatia
    ("CU", "CU-T"),  # Cuba
    ("CY", "5B-"),  # Cyprus
    ("CZ", "OK-"),  # Czechia
    ("DK", "OY-"),  # Denmark
    ("DK", "OY-H"),  # Denmark
    ("DK", "OY-X"),  # Denmark
    ("DK", "OY-B"),  # Denmark
    ("DJ", "J2-"),  # Djibouti
    ("DM", "J7-"),  # Dominica
    ("DO", "HI-"),  # Dominican Republic
    ("EC", "HC-"),  # Ecuador
    ("EG", "SU-"),  # Egypt
    ("SV", "YS-"),  # El Salvador
    ("GQ", "3C-"),  # Equatorial Guinea
    ("ER", "E3-"),  # Eritrea
    ("EE", "ES-"),  # Estonia
    ("ET", "ET-"),  # Ethiopia
    ("FK", "VP-F"),  # Falkland Islands (Malvinas)
    ("FO", "OY-"),  # Faroe Islands
    ("FJ", "DQ-"),  # Fiji
    ("FI", "OH-"),  # Finland
    ("FR", "F-"),  # France
    ("FR", "F-OG"),  # France
    ("FR", "F-O"),  # France
    ("GA", "TR-"),  # Gabon
    ("GM", "C5-"),  # Gambia
    ("GE", "4L-"),  # Georgia
    ("DE", "D-"),  # Germany
    ("GH", "9G-"),  # Ghana
    ("GI", "VP-G"),  # Gibraltar
    ("GR", "SX-"),  # Greece
    ("GL", "OY-"),  # Greenland
    ("GD", "J3-"),  # Grenada
    ("GT", "TG-"),  # Guatemala
    ("GN", "3X-"),  # Guinea
    ("GW", "J5-"),  # Guinea-Bissau
    ("GY", "8R-"),  # Guyana
    ("HT", "HH-"),  # Haiti
    ("HN", "HR-"),  # Honduras
    ("HU", "HA-"),  # Hungary
    ("IS", "TF-"),  # Iceland
    ("IN", "VT-"),  # India
    ("ID", "PK-"),  # Indonesia
    ("IR", "EP-"),  # Iran
    ("IQ", "YI-"),  # Iraq
    ("IE", "EI-"),  # Ireland
    ("IM", "M-"),  # Isle of Man
    ("IL", "4X-"),  # Israel
    ("IT", "I-"),  # Italy
    ("CI", "TU-"),  # Ivory Coast
    ("JM", "6Y-"),  # Jamaica
    ("JP", "JA-"),  # Japan
    ("JO", "JY-"),  # Jordan
    ("KZ", "UN-"),  # Kazakhstan
    ("KE", "5Y-"),  # Kenya
    ("KI", "T3-"),  # Kiribati
    ("KP", "P-"),  # Democratic People's Republic of Korea
    ("KR", "HL-"),  # Republic of Korea
    ("KW", "9K-"),  # Kuwait
    ("KG", "EX-"),  # Kyrgyzstan
    ("LA", "RDPL-"),  # Laos
    ("LV", "YL-"),  # Latvia
    ("LB", "OD-"),  # Lebanon
    ("LS", "7P-"),  # Lesotho
    ("LR", "A8-"),  # Liberia
    ("LY", "5A-"),  # Libya
    ("LT", "LY-"),  # Lithuania
    ("LU", "LX-"),  # Luxembourg
    ("MK", "Z3-"),  # Macedonia
    ("MG", "5R-"),  # Madagascar
    ("MW", "7Q-"),  # Malawi
    ("MY", "9M-"),  # Malaysia
    ("MV", "8Q-"),  # Maldives
    ("ML", "TZ-"),  # Mali
    ("MT", "9H-"),  # Malta
    ("MH", "V7-"),  # Marshall Islands
    ("MR", "5T-"),  # Mauritania
    ("MU", "3B-"),  # Mauritius
    ("MX", "XA-"),  # Mexico
    ("MX", "XB-"),  # Mexico
    ("MX", "XC-"),  # Mexico
    ("FM", "V6-"),  # Micronesia
    ("MD", "ER-"),  # Moldova
    ("MC", "3A-"),  # Monaco
    ("MN", "JU-"),  # Mongolia
    ("ME", "4O-"),  # Montenegro
    ("MS", "VP-M"),  # Montserrat
    ("MA", "CN-"),  # Morocco
    ("MZ", "C9-"),  # Mozambique
    ("MM", "XY-"),  # Myanmar
    ("MM", "XZ-"),  # Myanmar
    ("NA", "V5-"),  # Namibia
    ("NR", "C2-"),  # Nauru
    ("NP", "9N-"),  # Nepal
    ("NL", "PH-"),  # Netherlands
    ("NL", "PJ-"),  # Netherlands
    ("NZ", "ZK-"),  # New Zealand
    ("NZ", "ZL-"),  # New Zealand
    ("NZ", "ZM-"),  # New Zealand
    ("NI", "YN-"),  # Nicaragua
    ("NE", "5U-"),  # Niger
    ("NG", "5N-"),  # Nigeria
    ("NO", "LN-"),  # Norway
    ("OM", "A4O-"),  # Oman
    ("PK", "AP-"),  # Pakistan
    ("PA", "HP-"),  # Panama
    ("PG", "P2-"),  # Papua New Guinea
    ("PY", "ZP-"),  # Paraguay
    ("PE", "OB-"),  # Peru
    ("PH", "RPC-"),  # Philippines
    ("PL", "SP-"),  # Poland
    ("PT", "CR-"),  # Portugal
    ("PT", "CS-"),  # Portugal
    ("QA", "A7-"),  # Qatar
    ("RE", "F-OD"),  # Réunion
    ("RO", "YR-"),  # Romania
    ("RU", "RA-"),  # Russian Federation
    ("RW", "9XR-"),  # Rwanda
    ("SH", "VQ-H"),  # Saint Helena/Ascension
    ("KN", "V4-"),  # Saint Kitts and Nevis
    ("LC", "J6-"),  # Saint Lucia
    ("VC", "J8-"),  # Saint Vincent and the Grenadines
    ("WS", "5W-"),  # Samoa
    ("SM", "T7-"),  # San Marino
    ("ST", "S9-"),  # Sao Tome and Principe
    ("SA", "HZ-"),  # Saudi Arabia
    ("SN", "6V-"),  # Senegal
    ("SN", "6W-"),  # Senegal
    ("RS", "YU-"),  # Serbia
    ("SC", "S7-"),  # Seychelles
    ("SL", "9L-"),  # Sierra Leone
    ("SG", "9V-"),  # Singapore
    ("SK", "OM-"),  # Slovakia
    ("SI", "S5-"),  # Slovenia
    ("SB", "H4-"),  # Solomon Islands
    ("SO", "6O-"),  # Somalia
    ("ZA", "ZS-"),  # South Africa
    ("ZA", "ZT-"),  # South Africa
    ("ZA", "ZU-"),  # South Africa
    ("ES", "EC-"),  # Spain
    ("LK", "4R-"),  # Sri Lanka
    ("SD", "ST-"),  # Sudan
    ("SR", "PZ-"),  # Suriname
    ("SZ", "3D-"),  # Eswatini
    ("SE", "SE-"),  # Sweden
    ("CH", "HB-"),  # Switzerland
    ("SY", "YK-"),  # Syria
    ("PF", "F-OH"),  # French Polynesia
    ("TJ", "EY-"),  # Tajikistan
    ("TZ", "5H-"),  # Tanzania
    ("TH", "HS-"),  # Thailand
    ("TG", "5V-"),  # Togo
    ("TO", "A3-"),  # Tonga
    ("TT", "9Y-"),  # Trinidad and Tobago
    ("TN", "TS-"),  # Tunisia
    ("TR", "TC-"),  # Turkey
    ("TM", "EZ-"),  # Turkmenistan
    ("TC", "VQ-T"),  # Turks and Caicos
    ("TV", "T2-"),  # Tuvalu
    ("UG", "5X-"),  # Uganda
    ("UA", "UR-"),  # Ukraine
    ("AE", "A6-"),  # United Arab Emirates
    ("GB", "G-"),  # United Kingdom
    ("US", "N-"),  # United States of America
    ("UY", "CX-"),  # Uruguay
    ("UZ", "UK-"),  # Uzbekistan
    ("VU", "YJ-"),  # Vanuatu
    ("VE", "YV-"),  # Venezuela
    ("VN", "VN-"),  # Vietnam
    ("YE", "7O-"),  # Yemen
    ("ZM", "9J-"),  # Zambia
    ("ZW", "Z-"),  # Zimbabwe
)
# result = ['("%s", "%s"),  # %s' % (row[0], row[1]+'-' if row[1].count('-') == 0 else row[1] , row[2]) for row in df[['Alpha-2 code', 'Registration Prefix', 'Country']].values]
